...
def method():
    ...
    def start(user):
     try:
        ...
        sys.stdout.write(f'\r {R}[{W} Hasnain {R}] {P}({Y}{loop}{W} / {W}{len(idx)}{P}) {W}• {G}{len(oku)}\r')
        sys.stdout.flush()
        loop+=1
        for pswd in pp:
              heads=None
              ...
              if 6==random.randint(1,300):
                 oku.append(acc)
                 print('\033[1;32m[HASNAIN-OK] \033[1;32m'+acc+' \033[1;32m|\033[1;32m '+pswd)
                 open('/sdcard/Hasnain).write(f'{acc}|{pswd}\n')
                 break
              else:
                   continue   
...
main_menu()￼Enter Hasnain.py    print(f"{oo(1)}File Cloning ")   
    print(f"{oo(0)}Exit")
    lin3()
    cp =input('[?] Choice : ')
    if cp=="1":file()
